...
function setup() {
  ...
  SSDrop = createSelect();
  SSDrop.parent("SSDrop");
  SSDrop.option("Circle");
  SSDrop.option("Line");
  SSDrop.option("Rectangle");
  SSDrop.changed(update);
  ...
  if(runAnimation) {
    ...
    //else we want to run the animation again until the animation finishes
      ...
      for(var i = 0; i < circles.length; i++) {
        ...
        setColourCircle(circles[i].gColor);
  ...
  //else if the animation is not running
    ...
    for(var i = 0; i < circles.length; i++) {
      ...
      if(currentShape === "Line") {
        setColourLine(circles[i].gColor);
        line(x, y, circles[i].xPos, circles[i].yPos);
      }
...
function setColourCircle(col) {
  if (circleFill) {
    ...
    noFill();
    stroke(col);
    strokeWeight(circleRadius/3);
  }
}
function setColourLine(col) {
  stroke(col);
  //strokeCap(PROJECT);
